...
public class WikipediaBatchExtractor implements NotificationListener {
			...
			public void run() {
				while (true) {
					try {
						...
						Debug.println(String.format("q=%d r=%d p=%d %%=%.2f", queue.size(), noPagesRead,
	...
	private static Date markStart(String fileName) {
		...
		Debug.println(String.format("Parse start (%s): %s", startTime, fileName));
	...
	private void waitComplete() {
		...
		try {
			...
			while (noRunningTasks > 0 || !queue.isEmpty()) {
				...
				Debug.println("Objects left => " + queue.size());
				Debug.println("Tasks left => " + noRunningTasks);
	...
	private static void markComplete(String fileName, Date startTime) {
		...
		Debug.println(String.format("Parse complete (%s): %s", endTime, fileName));
		...
		int day = remaining;
		Debug.println(String.format("Time taken: %dd %d:%d:%d", day, hr, min, sec));
	}
