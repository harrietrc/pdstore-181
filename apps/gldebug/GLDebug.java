...
public class GLDebug {
	...
	JButton singleViewRunButton, singleViewStopButton, singleViewStepButton, singleViewContinueButton,
	...
	protected class StateJTree extends JTree
	{
                ...
                boolean hasFocus)
		{
			...
			if(node.getStateVariableName() == null)
			{// This should never be hit, only the root should have a null, and it's taken care of above
				assert(false);
				return "";
			}
	...
	protected class EventHandler implements ActionListener, TreeExpansionListener, TreeSelectionListener,
		...
		DebuggerProtocol.BreakEventListener, DebuggerProtocol.BreakpointListener
	{
		public void actionPerformed(ActionEvent event)
		{
			...
			else if(event.getSource() == singleViewStepButton)
			{
				if(debugProtocol != null)
				{
					updateStatusBar("Sent step request");
					debugProtocol.sendStep(seq++);
				}
				else
				{
					updateStatusBar("Error, debugger not connected");
			...
			else if(event.getSource() == singleViewConnectButton)
			{
				...
				try
				{
					...
					debugProtocol.addBreakEventListener(eventHandler);
			...
			else if(event.getSource() == singleViewLogButton)
			{
				messageLogDialog.showMessageDialog();
			}
		...
		public void processRunningRecieved(String processName, TimeStamp timeStamp)
		{
			...
			messageLogDialog.addMessageTimestamp(runningSessionName + "- session started\n");
		}
		...
		{
			messageLogDialog.addMessageTimestamp("Breakpoint hit on function: " + functionName + "\n");
		...
		public void breakEventHit(String functionName, String breakEvent)
		{
			messageLogDialog.addMessageTimestamp(breakEvent + " in " + functionName + "\n");
			updateStatusBar(breakEvent + " in " + functionName);
		}
	}
	
	protected class MessageLogDialog extends JDialog
	{
		protected JScrollPane messageScrollPane;
		protected JPanel messageTopLevelPane;
		protected JTextArea messageArea;
		protected BorderLayout topLevelLayout;
		
		public MessageLogDialog(JFrame parent)
		{
			super(parent, "Message Log", false);
			
			messageTopLevelPane = new JPanel();
			topLevelLayout = new BorderLayout();
			messageTopLevelPane.setLayout(topLevelLayout);
			...
			messageTopLevelPane.add(messageScrollPane, BorderLayout.CENTER);
			
			getContentPane().add(messageTopLevelPane);
		}
		...
		public void addMessageTimestamp(String message)
		{
			Timestamp stamp = new Timestamp(new java.util.Date().getTime());
			messageArea.append(stamp.toString() + "-" + message);
		}
	...
	protected class BreakpointDialog extends JDialog implements ActionListener
	{
		...
		protected JPanel breakpointTopLevelPane, breakpointListPane, bottomPane, breakEventsPane, buttonPane;
		protected BoxLayout breakpointListLayout, bottomPaneLayout, breakEventsLayout, buttonLayout;
		protected BorderLayout breakpointTopLevelLayout;
		protected JButton addBreakpointButton, okayButton;
		protected JCheckBox breakOnGLErrorCheckbox, breakOnCompileErrorCheckbox, breakOnLinkErrorCheckBox;
		...
		public BreakpointDialog(JFrame parent)
		{
			...
			bottomPaneLayout = new BoxLayout(bottomPane, BoxLayout.Y_AXIS);
			bottomPane.setLayout(bottomPaneLayout);
				
			buttonPane = new JPanel();
			buttonLayout = new BoxLayout(buttonPane, BoxLayout.X_AXIS);
			...
			buttonPane.add(addBreakpointButton);
			buttonPane.add(Box.createHorizontalGlue());
			buttonPane.add(okayButton);
			
			breakEventsPane = new JPanel();
			breakEventsLayout = new BoxLayout(breakEventsPane, BoxLayout.X_AXIS);
			breakEventsPane.setLayout(breakEventsLayout);
			
			breakOnGLErrorCheckbox = new JCheckBox("Break on OpenGL Error");
			breakOnCompileErrorCheckbox = new JCheckBox("Break on Shader Compile Error");
			breakOnLinkErrorCheckBox = new JCheckBox("Break on Shader Link Error");
			
			breakOnGLErrorCheckbox.addActionListener(this);
			breakOnCompileErrorCheckbox.addActionListener(this);
			breakOnLinkErrorCheckBox.addActionListener(this);
			
			breakEventsPane.add(breakOnGLErrorCheckbox);
			breakEventsPane.add(breakOnCompileErrorCheckbox);
			breakEventsPane.add(breakOnLinkErrorCheckBox);
			
			bottomPane.add(breakEventsPane);
			bottomPane.add(buttonPane);
			
			breakpointTopLevelPane.add(bottomPane, BorderLayout.SOUTH);
		...
		public void actionPerformed(ActionEvent e) 
		{
			...
			else if(e.getSource() == breakOnGLErrorCheckbox)
			{
				JCheckBox checkBox = (JCheckBox)e.getSource();
				
				debugProtocol.setBreakEvent(seq++, DebuggerProtocol.REQ_EVENT_GL_ERROR, checkBox.isSelected());
			}
			else if(e.getSource() == breakOnCompileErrorCheckbox)
			{
				JCheckBox checkBox = (JCheckBox)e.getSource();
				
				debugProtocol.setBreakEvent(seq++, DebuggerProtocol.REQ_EVENT_COMPILE_ERROR, checkBox.isSelected());
			}
			else if(e.getSource() == breakOnLinkErrorCheckBox)
			{
				JCheckBox checkBox = (JCheckBox)e.getSource();
				
				debugProtocol.setBreakEvent(seq++, DebuggerProtocol.REQ_EVENT_LINK_ERROR, checkBox.isSelected());
			}
		...
		public void refreshBreakEvents()
		{
			boolean[] breakOnEvents = debugProtocol.getBreakOnEvent();
			
			breakOnGLErrorCheckbox.setSelected(breakOnEvents[DebuggerProtocol.REQ_EVENT_GL_ERROR]);
			breakOnCompileErrorCheckbox.setSelected(breakOnEvents[DebuggerProtocol.REQ_EVENT_COMPILE_ERROR]);
			breakOnLinkErrorCheckBox.setSelected(breakOnEvents[DebuggerProtocol.REQ_EVENT_LINK_ERROR]);
			
			this.validate();
		}
		
		public void swingRefreshBreakpointList()
		{
			...
			{
				String breakFunction = keys.nextElement();
				boolean doBreak = debugProtocol.getBreakOn().get(breakFunction) == '1' ? true : false;				
				...
				checkbox = new BreakpointCheckbox(breakFunction);
				checkbox.setSelected(doBreak);
				checkbox.addActionListener(this);
				
				breakpointPane.add(checkbox);
				breakpointPane.add(new JLabel(breakFunction));
	...
	protected void recusiveAddStateToDatabase(State state, GUID stateVariableGUID, String concatStateVariableName, TreeSet<GUID> visitedNodes, GUID transaction)
	{
		...
		for(State s : state.children)
		{
			...
			store.ensureLink(transaction, stateVariableGUID, childStateVariableRole, childStateVariableGUID);
	...
	protected Color recursivelyCheckStateVariableValueListColour(DefaultMutableTreeNode treeNode, GUID transaction1, GUID transaction2)
	{
		...
		if(transaction1.equals(transaction2)) // if transactions are the same, all values are the same
		{
			return Color.BLACK;
		}
		else
		{
			...
			if(variableString1.equals(variableString2)) // If strings are the same
			{
				if(selectedTransaction1IsCurrent && selectedTransaction2IsCurrent) // if both selected states are from the current session
				{
					if(transaction1.earlier(transaction2))
					{
						if(store.linkChangedSince(transaction1, transaction2, (Object)treeNodeStateVariableGUID, stateVariableValueRole))
						{
					...
					{
						if(store.linkChangedSince(transaction2, transaction1, (Object)treeNodeStateVariableGUID, stateVariableValueRole))
						{
							returnColour = Color.MAGENTA;
						}
						else
				...
				else // if one or more selected states are not current
				{
					returnColour = Color.BLACK;
				}
			}
	...
	protected void updateEnabledButtons(DebuggerProtocol.DebuggerStatus status)
	{
		if(debugMode)
		{
			...
			singleViewStepButton.setEnabled(true);
		...
		if(debugProtocol == null)
		{
			...
			singleViewStepButton.setEnabled(false);
	...
	protected void setupSingleViewLeftPane()
	{
		...
		label.setAlignmentX(Component.CENTER_ALIGNMENT);
		//singleViewSessionSelectPane.add(label); // TODO: restore this functionality
		
		singleViewFilterField.setAlignmentX(Component.CENTER_ALIGNMENT);
		//singleViewSessionSelectPane.add(singleViewFilterField); // TODO: restore this functionality
	...
	protected void setupSingleViewMiddlePane()
	{
		...
		singleViewStateTree.setRootVisible(true);
	...
	protected void setupSingleView()
	{
		...
		singleViewStepButton = new JButton("Step");
		...
		singleViewLogButton = new JButton("Log");
		...
		singleViewButtonsPane.add(singleViewStepButton);
		...
		singleViewButtonsPane.add(singleViewLogButton);
		...
		singleViewStepButton.addActionListener(eventHandler);
		...
		singleViewLogButton.addActionListener(eventHandler);
	...
	public GLDebug(PDStore store, GUID historyID)
	{
		...
		messageLogDialog = new MessageLogDialog(windowFrame);
