...
public class Transform {
	...
	public Transform(PDStore cache, PDGeneratorApplication ga) {
		// cache = ga.getPDCache();
	...
	private void traverse_output(PDInstance element, PDRole parentRole,
			PDStore cache, PDGeneratorApplication ga, TreeWalker walker) {
		String indent = "";
		for(int i=walker.gaOutputCursor.size();i>0;--i) indent += " ";
		Debug.println(indent+element.getName()+", "+parentRole.getName(), "PDstore");
	 ...
	 *            run on
	 */
	private void cloneElement(PDInstance element, PDRole role, PDStore cache,
		...
		if (walker.gaOutputCursor.isEmpty()) {
			...
			PDInstance clone = cache.newInstance(element.getTypeId());
			clone.setName(element.getName());
			cache.addLink(walker.gaOutputCursor.peek().getId(), role.getId(),
					clone.getId());
			walker.gaOutputCursor.push(clone);
		}
	}
	...
	private ArrayList<PDInstance> executeMapping(PDMap map,
		...
		for (Object ii : inputMatches) {
			...
			Object element : cache.getInstances(input_instance,
					inputRole.getId())) {
		...
		for (PDInstance inputInstance : verifiedMatches) {
			...
			if (outputRoles.size() > 1) {
				...
				for (PDRole OR : outputRoles) {
					Collection<Object> _outputs = cache.getInstances(
							outputInstance, OR.getId());
			...
			Collection<Object> inputs = cache.getInstances(inputInstance,
					inputRole.getId());
			...
			for (Object i : inputs) {
				for (Object o : outputs) {
					...
					for (PDRole r : outputRoles) {
						Collection<Object> oi = cache.getInstances(
								outputInstance, r.getId());
