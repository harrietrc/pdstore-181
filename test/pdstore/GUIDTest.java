...
public class GUIDTest extends TestCase {
	...
	public final void testSetBranchId() {
		...
		assertTrue(transaction.getBranchID().equals(branch)); // branchId should
																// be earliest
																// transactionID
	}
	...
	public final void testMaxTransactionId() {
		...
		assertTrue(max.compareTo(transactionId) == 1);
		assertTrue(max.compareTo(branch) == 1); // branchId should be earliest
												// transactionID
	}
	...
	public final void testReverseFunction() {
		// TODO use asserts instead of println
		long time = 1;
		for (int i = 1; i < 65; ++i) {
			Debug.println(" Bit:  " + i);
			Debug.println(" In:  "
