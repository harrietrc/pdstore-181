...
public class SubQueryTest extends TestCase {
	...
	public final void testsimpleSubquery_test() {
		...
		Query mergeQuery = new Query(true, query1, query2, query3);

		Iterator<Assignment<GUID, Object, GUID>> assignmentIterator = mergeQuery
				.execute(null);
		int count = 0;
	...
	public final void testSubQueryfilter_test() {
		...
		Query mergeQuery = new Query(true, query1, query2);
	...
	public final void testSubQueryOptional_test() {
		...
		Query mergeQuery = new Query(true, lstfinal.toArray(new Query[lstfinal
				.size() - 1]));
