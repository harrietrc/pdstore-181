...
public class GUID implements Pairable<GUID>, Comparable<GUID>, Serializable {
	...
	static final long serialVersionUID = -3248984379645L;

	private static final GUID maxTransactionGUID = new GUID(
			UUIDGen.encodeTime(Long.MAX_VALUE), UUIDGen.getClockSeqAndNode())
			.getFirst();
	...
	public final static long maskForFirst = 0x0010000000000000L;
	...
	/**
	 * Creates an artificial GUID that contains the given integer (between 0 and
	 * 99) in the first byte, clearly visible for debugging purposes. The
	 * following bytes are all 00.
	 * 
	 * @param i
	 *            the integer the GUID should clearly contain at the beginning
	 *            (should be between 0 and 99)
	 */
	public GUID(int i) {
		this((i < 10) ? "0" + i + "000000000000000000000000000000" : i
				+ "000000000000000000000000000000");
	}

	/**
	 * Private copy constructor which is used for generating new GUIDs for a
	 * given branch further below.
	 * 
	 * @param id
	 *            GUID to copy from
	 */
	...
	public boolean equals(Object obj) {
		...
		return (getTime() == t.getTime())
				&& (getClockSeqAndNode() == t.getClockSeqAndNode());
	}
	...
	public static void setBranchId(GUID transactionId, GUID branchId) {
		transactionId
				.setClockSeqAndNode(transactionId.getClockSeqAndNode() & 0xFFFF000000000000L);
		long clockSeqAndNode2 = 0;
		if(branchId!=null) clockSeqAndNode2 = branchId.getClockSeqAndNode();
	...
	public GUID getPartner() {
		GUID partnerGUID = new GUID(this);
		// implement xor on last bit
		long lastbit = partnerGUID.getClockSeqAndNode() & maskForFirst;
		if (lastbit != 0) {
			partnerGUID.setClockSeqAndNode(partnerGUID.getClockSeqAndNode()
					& (~maskForFirst));
		} else {
			partnerGUID.setClockSeqAndNode(partnerGUID.getClockSeqAndNode()
					| maskForFirst);
	...
	public boolean earlier(GUID id) {
		return id != null && getLongTimestamp() < id.getLongTimestamp();
	}

	/**
	 * Returns true iff this GUID has a later timestamp than the given GUID id.
	 * null is considered earliest possible time
	 * 
	 * @param id
	 *            the given GUID
	...
	public int compareTo(GUID id) {
		...
		// if the GUIDs have the same timestamp, then compare by their
		// clockSeqAndNode
		if (clockSeqAndNode > id.clockSeqAndNode)
			return 1;
		if (clockSeqAndNode < id.clockSeqAndNode)
			return -1;
		return 0;
	...
	public String toString() {
		...
		if(debugStore==null) return toRawString();
		return debugStore.getLabel(this);
	}
	...
	@Override
	public int hashCode() {
		return (int) ((getTime() >> 32) ^ getTime()
				^ (getClockSeqAndNode() >> 32) ^ getClockSeqAndNode());
	}
	...
	public static String toHexString(byte[] res) {
		...
		for (int i = 0; i < res.length; i++) {
			result.append(Integer.toString(((res[i] & 0xf0) >> 4) + 0x10, 16)
					.substring(1));
			result.append(Integer.toString((res[i] & 0x0f) + 0x10, 16)
					.substring(1));
		}
