 ...
 * @param <E>
 */
public class CartesianPowerIterator<E> extends
		StatefulIterator<List<E>> {
	Iterable<E> input;
	int exponent;
	...
	protected List<E> computeNext() {
		...
		if (isFirstStep) {
			...
			for (changingIndex = 0; changingIndex < exponent; changingIndex++)
				if (product.get(changingIndex) != null
						&& product.get(changingIndex).hasNext()) {
					break;
				}
	...
	public long getStatistics(int whereIndex) {
		return changesProcessed[whereIndex];
	}

	public void incrementStatistics(int changingIndex) {
		// TODO remove this? inline this?
