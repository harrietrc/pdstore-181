...
public class ChangeLogStore implements PDCoreI<GUID, Object, GUID> {
	...
	public static final GUID VERSION_GUID = new GUID(
			"160060ea5b7ade11a67eb494a0878937");
	...
	public ChangeLogStore(String fileName) {
		try {
			...
			throw new PDStoreException("Error opening file " + fileName, e);
		}
	}
	...
	public GUID getVersion() throws IOException {
		...
		return new GUID(GUID.toHexString(versionGUID));
	}
	...
	public GUID getRepository() throws PDStoreException {
		try {
			...
			return new GUID(GUID.toHexString(repositoryGUID));
		} catch (IOException e) {
	...
	private GUID getMasterBranchTransaction() throws PDStoreException {
		try {
			...
			file.seek(0);
			return new GUID(GUID.toHexString(MasterBranchGUID));
		} catch (IOException e) {
	...
	Object readInstance(PrimitiveType type) throws IOException {
		...
		case GUID: {
			...
			file.read(bytes);
			return new GUID(GUID.toHexString(bytes));
		}
		case INTEGER: {
		...
		case BLOB: {
			...
			return new Blob(fileName, position, length);
		}
		case BOOLEAN: {
	...
	private void writeInstance(Object instance) throws IOException {
		...
		case BLOB: {
			// update Blob object to include correct log file and log position
			// for later loading
			Blob blob = (Blob) instance;
			blob.setFileName(fileName);
			blob.setPosition(file.getFilePointer());
	...
	public static byte[] createNullByteArray(int arrayLength) {
	...
	private List<PDListener<GUID, Object, GUID>> listenerList = new CopyOnWriteArrayList<PDListener<GUID, Object, GUID>>();
	...
	public GUID addTransaction(Transaction<GUID, Object, GUID> transaction)
		...
		// old comment: addTrasnaction does not seem to honor change.isInLog(),
		// or does it?
		// Answer it does, but:
		// TODO: there must be more test cases in ChangeLogStore.
		// they must write and read changes that should not be in log.
		...
		for (PDChange<GUID, Object, GUID> change : transaction) {
			if (change.isInLog()
				// Make sure, not only isolationlevel is committed
				&& !PDStore.ISOLATIONLEVEL_ROLEID.equals(change.getRole2())) {
				lastChange = count;
			}
			++count;
		}

		// If there is no change then nothing, not even a transaction header
		...
		synchronized (this) {
			...
			// T0D0 done: Gerald, please check that the role is used correctly
			// yes, this is the point that defines what is the right way.
