...
public class SortByOrder<TransactionType extends Comparable<TransactionType>, InstanceType, RoleType extends Pairable<RoleType>>
	...
	public <E extends InstanceType> List<E> sort(Collection<E> c) {
		...
		if (c.size() != 0) {
			...
			GenericPDStore<TransactionType, InstanceType, RoleType> workingCopy = (GenericPDStore<TransactionType, InstanceType, RoleType>) ((PDInstance) element)
					.getStore();

			// Sort forwards from first InstanceType.
			E next;
			while ((next = (E) workingCopy.getInstance(
					(InstanceType) ((PDInstance) sorted.get(sorted.size() - 1))
							.getId(), this.nextRole)) != null) {
				sorted.add(next);
			}
			...
			while ((previous = (E) workingCopy.getInstance(
					(InstanceType) ((PDInstance) sorted.get(0)).getId(),
					previousRole)) != null) {
