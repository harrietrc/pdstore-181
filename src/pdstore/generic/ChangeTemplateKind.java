...
/**
 * Encodes the different cases of unknowns in a change template. I means
 * instance1 (or instance2) are known. R means the role2 is known. X means the
 * respective component is unknown.
 * 
 * @author clut002
 * 
 */
public enum ChangeTemplateKind {
	...
	/**
	 * @param change
	 * @param assignedVariavbles
	 * @return
	 */
	public static ChangeTemplateKind getKind(PDChange<?, ?, ?> change,
			Set<Variable> assignedVariavbles) {
		int code = 0;
		if (!isUnspecified(change.getInstance1(), assignedVariavbles))
			code += 1;
		if (!isUnspecified(change.getRole2(), assignedVariavbles))
			code += 2;
		if (!isUnspecified(change.getInstance2(), assignedVariavbles))
			code += 4;
	...
	static boolean isUnspecified(Object o, Set<Variable> assignedVariavbles) {

		return o == null
				|| ((o instanceof Variable) && !assignedVariavbles.contains(o));
	}
}
