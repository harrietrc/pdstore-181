...
public class PDSimpleWorkingCopy implements PDWorkingCopy {
	...
	public PDInstance load(GUID typeId, GUID instanceId)
		...
		PDInstance i = null;
		Class<?> dalClass = PDStore.getDataClass(typeId);
		if (dalClass != null) {
			...
			if (i == null) {
				...
				try {
					...
					i = (PDInstance) constr.newInstance(store, instanceId);
	...
	private Object getProperInstanceObject(PDInstance instance1, GUID role2,
		...
		// if type is Object or Any, then the type is determined dynamically
		// using the has-type relation in load
		if (type2Id.equals(PDStore.OBJECT_TYPEID)
				|| type2Id.equals(PDStore.ANY_TYPEID)) {
			try {
				return load(instance2Id);
			} catch (PDStoreException e) {
				// If there is no has-type link, we don't know what type the
	 ...
	 * @throws PDStoreException
	 */
	public String getName(Object instanceId) throws PDStoreException {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see pdstore.PDWorkingCopy#setName(java.lang.Object, java.lang.String)
	 */
	public void setName(Object instance, String name) throws PDStoreException {
		store.setName(getTransaction(), instance, name);
	}
