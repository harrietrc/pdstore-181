...
public abstract class GenericConcurrentStore<TransactionType extends Comparable<TransactionType>, InstanceType, RoleType extends Pairable<RoleType>>
	...
	protected Map<TransactionType, Transaction<TransactionType, InstanceType, RoleType>> idToTransaction = new ConcurrentHashMap<TransactionType, Transaction<TransactionType, InstanceType, RoleType>>();
	...
	public Collection<PDChange<TransactionType, InstanceType, RoleType>> getChanges(
		...
		// if transaction is unspecified, then look at all committed
		// transactions
		if (transaction == null || transaction instanceof Variable) {
			committedStoreTemplate
					.setTransaction(getLatestDurableTransaction());
			return committedTransactionStore.getChanges(committedStoreTemplate);
		}

		// if given transaction has already been committed, look only into
		// committedTransactionStore
		...
		Transaction<TransactionType, InstanceType, RoleType> transactionObject = idToTransaction
				.get(transaction);
		if (transactionObject == null
				|| transactionObject.getIsolationLevel()
						.seesLatestDurableTransaction()) {
			// use the latest transaction on this branch for querying the
			// committed transaction store.
			Debug.println("seesLatestDurableTransaction", "ReadUncommitted");
			committedStoreTemplate.setTransaction(typeAdapter
					.maxTransactionId(transaction));
		}
		...
		if (transactionObject != null
				&& transactionObject.getIsolationLevel().isDirtyReading()) {
			// use a maximum transacion ID on the branch for querying the open
			// transaction store.
			Debug.println("isDirtyReading", "ReadUncommitted");
			openStoreTemplate.setTransaction(typeAdapter
		...
		if (!change.getChangeType().equals(ChangeType.LINK_EFFECTIVE))
			return uncommittedChanges;
		AggregationIterator<TransactionType, InstanceType, RoleType> aggregationIterator = new AggregationIterator<TransactionType, InstanceType, RoleType>(
				uncommittedChanges.iterator(), false);
